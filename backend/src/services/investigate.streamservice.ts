...
export const InvestigateStreamService = async (
  ...
  if (errorLogs.length > 0) {
    findings.push(`${errorLogs.length} ERROR logs detected after deployment`);
  }
  ...
  if (maxLatency > 500) {
    findings.push(
      `Latency increased from normal levels to ${maxLatency}ms after deployment`,
    );
  }

  if (maxErrorRate > 5) {
    findings.push(`Error rate peaked at ${maxErrorRate}% after deployment`);
  }
